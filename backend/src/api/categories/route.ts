...
type CategoryService = {
  ...
  createCategories: (data: { name: string; slug: string; parent_id?: string | null }) => Promise<CategoryRecord[]>,
}
...
export const POST = async (req: MedusaRequest, res: MedusaResponse): Promise<void> => {
  const { name, slug, parentId } = (req.body as { name?: string; slug?: string; parentId?: string | null }) || {}
